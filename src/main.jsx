...
function App(){const [view,setView]=useState('loading');const [auth,setAuth]=useState(null);const [user,setUser]=useState(null);useEffect(()=>{fetch('/api/me').then(async r=>r.ok?(await r.json()).user:null).then(u=>{if(u){setUser(u);setView('dashboard')}else setView('landing')}).catch(()=>setView('landing'))},[]);const success=u=>{setUser(u);setAuth(null);setView('dashboard')};if(view==='loading')return <div className="boot-screen"><Logo/><span>Opening your protected workspace…</span></div>;return <>{view==='dashboard'?<Dashboard user={user} onLogout={()=>{setUser(null);setView('landing')}}/>:<Landing onStart={()=>setAuth('register')} onLogin={()=>setAuth('login')}/>} {auth&&<Auth mode={auth} setMode={setAuth} onSuccess={success} onClose={()=>setAuth(null)}/>} {user&&!user.onboardingComplete&&<Onboarding user={user} onDone={setUser}/>}</>}
