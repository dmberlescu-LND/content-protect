...
- A new schema-bound, non-destructive production retention preview completed successfully on 20 July 2026 against revision `866888dfdfbe` and migration `019_incident_register.sql`. It found no customer/content, evidence-capture or closed-security-incident deletion candidates and only 29 expired rate-limit entries. Render ran `node scripts/retention.mjs` with `execute: false`; the transaction rolled back, no database row or storage object was deleted, and execution remains disabled pending policy, legal and operational approval.
