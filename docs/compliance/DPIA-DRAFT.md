...
- Reported-party dispute intake: case reference, exact reported URL, safe contact email, country, reason, statement and optional public HTTPS support link. Complete intake is application-encrypted; ordinary views receive only minimised metadata and checksums.
...
| Malicious intake, case enumeration or exposure of a counter-notice | High         | Generic unknown-case responses, delivered-case and exact-URL binding, distributed limits, honeypot, no file upload, application encryption, keyed contact pseudonym, metadata-only views, TOTP-gated operator access and audited outcome | Security + Legal      |
